...
public class Weapon : MonoBehaviour
{
    ...
    protected List<Enemy> enemyDamaged;
    ...
    protected virtual void Attack()
    {
        ...
        {
            if (!enemyDamaged.Contains(enemy.GetComponent<Enemy>()))
            {
                Debug.Log("Enemy be attacked");
                // Xử lí quái được nhận dâmge
                enemyDamaged.Add(enemy.GetComponent<Enemy>());
            }
        }
    ...
    protected virtual Enemy GetEnemy()
    {
        ...
        {
            Enemy enemyChecked = enm.GetComponent<Enemy>();
            float distance = 100;
            if(enemyChecked!=null) 
                distance = Vector2.Distance(transform.position, enemyChecked.transform.position);
